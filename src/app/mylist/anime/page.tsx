    ...
    return (
        <>
            <main className="pt-32 pb-16 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto min-h-screen">
                ...
                {!hobbies || hobbies.length === 0 ? (
                    <div className="border border-outline bg-surface-container p-16 text-center">
                        ...
                        <h3 className="font-display text-[24px] uppercase tracking-[1.5px] text-text-primary mb-2">{dict.noAnime}</h3>
                    ...
                    <>
                            ...
                            {(hobbies as Hobby[]).map((item) => (
                                <div key={item.id} className="bg-surface-container rounded-none overflow-hidden flex flex-col group border border-outline hover:border-secondary transition-all duration-300 transform hover:scale-[1.02]">
                                    <div className="relative h-80 overflow-hidden bg-background border-b border-outline">
                                        ...
                                        ></div>
                                        <div className="absolute top-4 right-4 bg-transparent border border-primary text-primary px-3 py-1 rounded-none font-label-md uppercase tracking-[2px] text-[10px] font-bold flex items-center gap-1 backdrop-blur-md">
                                    ...
                                    <div className="p-8 flex flex-col flex-grow relative z-10">
                                        <div className="flex flex-wrap gap-2 mb-4">
                                            {item.genre && (
                                                <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 border border-outline-variant text-text-secondary rounded-none group-hover:text-secondary group-hover:border-secondary transition-colors">
                                                    {item.genre.split(',')[0]}
                                                </span>
                                            )}
                                            {item.year && (
                                                <span className="text-[10px] uppercase tracking-widest font-bold px-2 py-0.5 border border-outline-variant text-text-secondary rounded-none group-hover:text-secondary group-hover:border-secondary transition-colors inline-block group-hover:scale-105 transition-all duration-300">
                                                    {item.year}
                                                </span>
                                            )}
                                        </div>
                                        <h3 className="font-display text-[24px] uppercase tracking-[1.5px] text-text-primary mb-3 group-hover:text-secondary transition-colors line-clamp-2 inline-block group-hover:scale-105 origin-left transition-all duration-300">{item.title}</h3>
                                        <p className="font-body-md text-text-secondary line-clamp-3 mb-8 flex-grow group-hover:text-secondary transition-colors inline-block group-hover:scale-105 origin-left transition-all duration-300">
                                        ...
                                        <div className="pt-6 border-t border-outline flex justify-between items-center">
                                            <span className="font-label-md uppercase tracking-[2px] text-[11px] flex items-center gap-2 text-primary">
                                                ...
                                                {item.watching_status === 'watching' && <><span className="material-symbols-outlined text-[16px]">play_arrow</span> {dict.watching}</>}
                        ...
                        <div className="mt-16 flex items-center justify-center gap-4">
                            <Link
                                ...
                                className={`flex items-center justify-center w-12 h-12 rounded-full border border-white/10 transition-all duration-300 transform ${page <= 1 ? "pointer-events-none opacity-30" : "hover:border-secondary hover:text-secondary hover:scale-105 text-white"}`}
                            >
                            ...
                            <Link
                                href={`?page=${page + 1}${selectedStatus ? `&status=${encodeURIComponent(selectedStatus)}` : ""}`}
                                className={`flex items-center justify-center w-12 h-12 rounded-full border border-white/10 transition-all duration-300 transform ${(hobbies?.length ?? 0) < PAGE_SIZE ? "pointer-events-none opacity-30" : "hover:border-secondary hover:text-secondary hover:scale-105 text-white"}`}
                            >
            ...
            <section className="py-24 px-margin-mobile md:px-margin-desktop max-w-container-max mx-auto border-t border-outline mt-32">
                <div className="bg-background border border-outline rounded-none p-8 md:p-16 relative overflow-hidden">
                                ...
                                <div key={`req-${item.id}`} className="bg-surface-container border border-outline rounded-none overflow-hidden flex flex-col hover:border-secondary transition-all duration-300 transform hover:scale-[1.02] group">
                                    <div className="flex items-start gap-6 p-6">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <h3 className="font-display text-[20px] uppercase tracking-[1.5px] text-text-primary mb-2 group-hover:text-secondary group-hover:scale-105 origin-left inline-block transition-all duration-300">{item.title}</h3>
                                            <div className="flex gap-4 mb-4 font-label-md uppercase tracking-[2px] text-[10px] text-text-secondary">
                                                <span className="bg-surface-container border border-outline px-3 py-1 group-hover:border-secondary group-hover:text-secondary group-hover:scale-105 transition-all duration-300 inline-block">{item.year}</span>
                                                <span className="bg-surface-container border border-outline px-3 py-1 group-hover:border-secondary group-hover:text-secondary group-hover:scale-105 transition-all duration-300 inline-block">{item.genre}</span>
                                            </div>
                                            <p className="font-body-md text-text-secondary line-clamp-3 mb-6 group-hover:text-secondary group-hover:scale-105 origin-left inline-block transition-all duration-300">{item.synopsis || dict.recBySomeone}</p>
                                        </div>
                                    </div>
