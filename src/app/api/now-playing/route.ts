...
export async function GET() {
    ...
    try {
        ...
        // Jika ada lagu yang sedang diputar
        ...
        // Jika tidak ada lagu yang diputar, ambil lagu terakhir
        const recentlyPlayedRes = await fetch('https://api.spotify.com/v1/me/player/currently-playing', {
            headers: { Authorization: `Bearer ${accessToken}` },
        });
        ...
        if (recentlyPlayedRes.ok) {
            ...
            const trackData = {
                isPlaying: false,
                name: lastTrack.name,
                artists: lastTrack.artists.map((artist: any) => artist.name),
                album: { images: lastTrack.album.images },
            };
            return NextResponse.json(trackData);
        }
