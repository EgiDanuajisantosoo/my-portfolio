    ...
    <div className="sticky top-0 z-50 w-full bg-neutral-800/90 backdrop-blur-md p-4 border-b border-neutral-700 shadow">
      ...
      <div className="hidden md:flex flex-wrap items-center gap-6 justify-between">
        ...
        <div className="flex gap-3">
          ...
          <button onClick={() => onTimeRangeChange('long_term')} className={`${baseButtonClass} ${timeRange === 'long_term' ? 'bg-green-500' : 'bg-neutral-600 hover:bg-neutral-500'}`}>Selamanya</button>
        </div>
      </div>
    ...
    return (
        <li className="flex items-center gap-4 rounded p-2 transition-colors hover:bg-neutral-800">
            ...
            <div>
                ...
                <p className="text-sm capitalize text-neutral-400">{artist.genres?.join(', ') || 'No Genre'}</p>
            </div>
        </li>
    );
};
...
export default function TopItemsPage() {
    ...
    return (
        <div className="w-full max-w-4xl mx-auto p-4">
            <h1 className="text-3xl font-bold">Top Spotify Anda</h1>
            <p className="text-neutral-400 mt-1">Lihat artis dan lagu yang paling sering Anda putar.</p>
