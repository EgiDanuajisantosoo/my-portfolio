...
export default async function BookPage({ searchParams }: any) {
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">
            Cari buku favoritmu menggunakan {source === "google" ? "Google Books" : "Open Library"}.
          </p>
        </div>
      ...
      >
        <div className="relative flex-1 w-full">
          <input
            ...
            name="q"
            defaultValue={query}
            placeholder="Cari buku (contoh: Laskar Pelangi, Harry Potter...)"
            ...
            return (
              ...
              >
                ...
                <div className="p-4 flex flex-col flex-1">
                  ...
                  {/* Genre */}
                  <p className="text-[10px] text-blue-300 mt-2 line-clamp-1 bg-blue-500/10 px-2 py-1 rounded inline-block w-fit">
                    {book.genre || "Buku"}
                  </p>
          ...
          <div className="border border-dashed rounded-xl p-10 text-center text-gray-400 bg-white/5 backdrop-blur-sm border-white/20 mb-10">
            {query ? (
              <>
                ...
                {source === "google" ? (
                  <a href={`/add-hobby/buku?q=${encodeURIComponent(query)}&source=oplib`} className="inline-flex items-center gap-2 bg-blue-600/20 text-blue-400 hover:bg-blue-600/40 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-blue-500/30">
                    <span className="material-symbols-outlined text-[18px]">travel_explore</span>
                    Coba cari di Open Library
                  </a>
                ) : (
                  <a href={`/add-hobby/buku?q=${encodeURIComponent(query)}&source=google`} className="inline-flex items-center gap-2 bg-green-600/20 text-green-400 hover:bg-green-600/40 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-green-500/30">
                    <span className="material-symbols-outlined text-[18px]">travel_explore</span>
              ...
              <>
                <p className="font-medium text-white">Cari Buku Favorit Anda</p>
                <p className="text-sm mt-1">Ketik judul buku atau penulis di kolom pencarian di atas.</p>
              </>
            )}
          </div>
          ...
          {trendingBooks.length > 0 && (
            <div>
              <div className="flex items-center gap-3 mb-6">
                ...
                <h3 className="font-semibold text-lg text-white">Rekomendasi Buku Terpopuler (via Open Library)</h3>
              </div>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-6">
